...
const props = defineProps<{ curveData: any }>();
const chartRef = ref();
  ...
  tooltip: {
    ...
    formatter: (p: any) => {
      ...
      color:#fff; background: #6B9DFE;border-radius: 4px;font-size:14px; ">
        <div style="display: flex; align-items: center;"> <div style="width:5px;height:5px;background:#ffffff;border-radius: 50%;margin-right:5px"></div>菜品 :  ${p[0].name}</div>
        <div style="display: flex;align-items: center;"><div style="width:5px;height:5px;background:#ffffff;border-radius: 50%;margin-right:5px"></div>销售量 :  ${p[0].value}</div>
      </div>`;
      return dom;
    }
  },
